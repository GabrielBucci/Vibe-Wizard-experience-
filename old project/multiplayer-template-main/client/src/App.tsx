...
type Projectile = moduleBindings.Projectile;
...
function App() {
  ...
  const [projectiles, setProjectiles] = useState<ReadonlyMap<number, Projectile>>(new Map());
  ...
  const registerProjectileCallbacks = useCallback(() => {
    if (!conn) return;
    console.log("Registering projectile callbacks...");

    conn.db.projectile.onInsert((_ctx: EventContext, projectile: Projectile) => {
      console.log("Projectile inserted:", projectile.id);
      setProjectiles(prev => new Map(prev).set(projectile.id, projectile));
    });

    conn.db.projectile.onUpdate((_ctx: EventContext, _old: Projectile, newProjectile: Projectile) => {
      setProjectiles(prev => new Map(prev).set(newProjectile.id, newProjectile));
    });

    conn.db.projectile.onDelete((_ctx: EventContext, projectile: Projectile) => {
  ...
  }, [identity, onSubscriptionApplied, onSubscriptionError]);
  ...
  // NEW: Add cast projectile handler
  const handleCastProjectile = useCallback((direction: THREE.Vector3, handPosition: THREE.Vector3) => {
    if (!conn || !identity || !localPlayer) return;

    // Check cooldown
    const now = Date.now();
    const lastCastTime = localPlayer.last_cast_time.micros() / 1000; // Convert to milliseconds
    const cooldownMs = moduleBindings.PROJECTILE_COOLDOWN_SECS * 1000;
    
    if (now - lastCastTime < cooldownMs) {
      console.log("Spell on cooldown");
      return;
    }

    // Call the cast_projectile reducer
    conn.reducers.castProjectile(
      handPosition.x,
      handPosition.y,
      handPosition.z,
      direction.x,
    ...
    // Update input state to trigger casting animation
    currentInputRef.current = {
      ...currentInputRef.current,
      castSpell: true
    };
  }, [identity, localPlayer]);
      ...
      {connected && (
        <>
          <GameScene 
            ...
            onCastProjectile={handleCastProjectile}
          />
