...
interface GameSceneProps {
  ...
  projectiles: ReadonlyMap<number, any>; // Temporarily use 'any' until we regenerate bindings
  ...
  onCastProjectile?: (direction: THREE.Vector3, handPosition: THREE.Vector3) => void;
}

const ProjectileVisual: React.FC<{ projectile: any }> = ({ projectile }) => {
  return (
    <mesh
      position={[
        projectile.data.position.x,
        projectile.data.position.y,
        projectile.data.position.z
      ]}
      quaternion={new THREE.Quaternion(
        projectile.data.rotation.x,
        projectile.data.rotation.y,
        projectile.data.rotation.z,
        projectile.data.rotation.w
      )}
      castShadow
    >
...
const SceneContent: React.FC<GameSceneProps & { directionalLightRef: React.RefObject<THREE.DirectionalLight> }> = ({
  players,
  projectiles,
  localPlayerIdentity,
  onPlayerRotation,
  ...
  useEffect(() => {
    const handleMouseDown = (event: MouseEvent) => {
      if (event.button === 0 && onCastProjectile) {
        const camera = three.camera;
        const direction = new THREE.Vector3();
        camera.getWorldDirection(direction);
        
        const localPlayer = Array.from(players.values()).find(
          p => p.identity.toHexString() === localPlayerIdentity?.toHexString()
        );
        ...
        if (localPlayer) {
          ...
          onCastProjectile(direction, handPosition);
        }
      }
    };
  ...
  }, [onCastProjectile, players, localPlayerIdentity, three.camera]);
  ...
  return (
    <>
      ...
      <Grid 
        ...
        cellSize={2} 
        cellThickness={1}
        cellColor={new THREE.Color('#666666')} // Slightly darker grid lines
        sectionColor={new THREE.Color('#888888')} // Section lines
        fadeDistance={100}
        fadeStrength={1}
      />
...
export const GameScene: React.FC<GameSceneProps> = (props) => {
  ...
  return (
    <Canvas 
      ...
      shadows
    >
      <SceneContent {...props} directionalLightRef={directionalLightRef} />
    </Canvas>
  );
};
